            ...
            <div class="page-title-box">
                <div class="page-title-right">
                    <ol class="breadcrumb m-0">
                        ...
                        <li class="breadcrumb-item active">@lang('templates.add new data')</li>
                    </ol>
                </div>
                <h4 class="page-title">@lang('templates.add new data')</h4>
            </div>
        </div>
    </div>
    <div class="card">
        ...
        <div class="card-body">
            <form action="{{ url('/datas/' . $customerId . '/store/' . $templateId) }}" method="post">
                {{ csrf_field() }}
                <div class="row">
                    <div class="col-md-10">
                        @foreach ($details as $detail)
                            <div class="form-group row mb-3">
                                @if ($detail->type == 'Text')
                                    ...
                                    <div class="col-md-9">
                                        <input type="text" maxlength="256" class="form-control" name="{{ $detail->name }}" required>
                                    </div>
                                @elseif ($detail->type == 'Integer')
                                    <label class="col-md-3 col-form-label text-right" for="{{ $detail->name }}">{{ $detail->name }}</label>
                                    <div class="col-md-9">
                                        <input type="number" class="form-control" name="{{ $detail->name }}" min=0 max=99999999999999999999 step=1 retuired>
                                    </div>
                                @elseif ($detail->type == 'Date')
                                    <label class="col-md-3 col-form-label text-right" for="{{ $detail->name }}">{{ $detail->name }}</label>
                                    <div class="col-md-9">
                                        <input type="text" name="{{ $detail->name }}" class="form-control flatpickr-input-date" readonly="readonly" retuired>
                                    </div>
                                @elseif ($detail->type == 'Time')
                                    <label class="col-md-3 col-form-label text-right" for="{{ $detail->name }}">{{ $detail->name }}</label>
                                    <div class="col-md-9">
                                        <input type="text" name="{{ $detail->name }}" class="form-control flatpickr-input-time" readonly="readonly" required>
                                    </div>
                                @elseif ($detail->type == 'CheckBox')
                                    ...
                                    <div class="col-md-9">
                                        <div class="custom-control checkbox checkbox-warning">
                                            <input name="{{ $detail->name }}" type="checkbox" class="custom-control-input" id="{{ $detail->id }}">
